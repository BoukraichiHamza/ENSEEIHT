...
s(find(s == 0)) = 1 ; % s(i)== 1 si b(i) >=0 , s(i)== -1 si b(i) < 0
A = s.*A;
b = s.*b;
%
%% PHASE I
...
while any(OPT*pentes > 0) % Y-a-t-il un chemin améliorant ?
    ...
    JNP =     %JNP : indices j des vecteurs Aj définissant des chemins améliorant (xj > 0)
    ...
    [~,indalpha] = max(??); %indalpha : indice dans pente du chemin améliorant de plus forte pente
