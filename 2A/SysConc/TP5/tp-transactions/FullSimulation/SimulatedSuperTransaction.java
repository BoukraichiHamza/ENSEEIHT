...
// Une super transaction qui se relance à chaque avortement,
// jusqu'à soit terminer ou atteindre sa borne supérieure d'essais.

public class SimulatedSuperTransaction extends AbstractTransaction {
    ...
    // Le nombre de fois où la SuperTransaction a été relancée.
    private int nbRetry;
    // Le booléen de sortie de boucle.
    private boolean committed;
    // Le générateur de nombres aléatoires qui permet de simuler les
    // écritures.
    private Random rand;

    public SimulatedSuperTransaction(
        String name,
        TMAbstract tm,
    ...
    public void aborted() {
        try {
            if (this.nbRetry < 20) {
                SimulatedSuperTransaction next =
                    new SimulatedSuperTransaction(
                            this.name,
                            this.tm,
                            this.instructions,
                            this.response,
