...
class writeListener implements ActionListener {
	public void actionPerformed (ActionEvent ae) {
		try {
			...
			Irc.producer.send( Irc.session.createTextMessage(Irc.myName +" : " + Irc.data.getText()));
				Irc.data.setText("");
...
class connectListener implements ActionListener {
	public void actionPerformed (ActionEvent ae) {
		...
		MessageListener listener = new MessageListener() {
        			...
        			String texte = textmsg.getText();
        			String [] splitedText = new String[2];
        			splitedText = texte.split(" : ");
        			if (! splitedText[0].equals(Irc.myName)) {
						if (splitedText[1].equals("WHO")) {
							Irc.producer.send( Irc.session.createTextMessage(Irc.myName +" : " + Irc.myName));
						} else {
...
class whoListener implements ActionListener {
	public void actionPerformed (ActionEvent ae) {
		try {
			...
			Irc.producer.send( Irc.session.createTextMessage(Irc.myName +" : " + "WHO"));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
